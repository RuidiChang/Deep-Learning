    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for l in range(self.num_layers):
                hidden[l] = self.rnn[l].forward(h_prev, self.hiddens[i][l])
                h_prev = hidden[l]
            self.hiddens.append(hidden.copy())
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len-1,-1,-1):
            for l in range(self.num_layers-1,-1,-1):
                h_prev_t=self.hiddens[i][l]
                if l == 0:
                    h_prev_l = self.x[:,i,:]
                else:
                    h_prev_l = self.hiddens[i+1][l-1]
                dx_l, dh_l = self.rnn[l].backward(dh[l], self.hiddens[i+1][l], h_prev_l, h_prev_t)
                dh[l] = dh_l
                if l!=0:
                    dh[l-1]+=dx_l
