...
class Adam():
    def __init__(self, model, lr, beta1=0.9, beta2=0.999, eps=1e-8):
        ...
        self.m_W = [np.zeros(l.W.shape, dtype="float32") for l in model.layers]
        self.v_W = [np.zeros(l.W.shape, dtype="float32") for l in model.layers]

        self.m_b = [np.zeros(l.b.shape, dtype="float32") for l in model.layers]
        self.v_b = [np.zeros(l.b.shape, dtype="float32") for l in model.layers]
