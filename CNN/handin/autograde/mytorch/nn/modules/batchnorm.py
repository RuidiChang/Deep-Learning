...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdM        = (-1)*np.sum(dLdNZ*(self.V+self.eps)**(-0.5),axis=0)-(2/self.N)*dLdV*np.sum(self.Z-self.M,axis=0) # TODO
        
        dLdZ        = dLdNZ*((self.V+self.eps)**(-0.5))+dLdV*((2/self.N)*(self.Z-self.M))+dLdM*(1/self.N) # TODO
